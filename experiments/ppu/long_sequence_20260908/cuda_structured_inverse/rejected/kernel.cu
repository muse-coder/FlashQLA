...
// One participating warp computes one 16x16 tile of the inverse composition.
// Absolute shared coordinates retain the original 64-column swizzled layout.
template<int K> __device__ __forceinline__ void inverse_tile(const bf16* a,const bf16* b,int ar,int ac,int br,int bc,float (&z)[8]){
 int lane=threadIdx.x&31;
 #pragma unroll
 for(int e=0;e<8;e++)z[e]=0.f;
 #pragma unroll
 for(int kk=0;kk<K;kk+=16){
  unsigned ar_regs[4],br_regs[4];
  unsigned pa=(unsigned)__cvta_generic_to_shared(a+sw(ar+(lane&15),ac+kk+(lane>>4)*8,64));
  unsigned pb=(unsigned)__cvta_generic_to_shared(b+sw(br+kk+(lane&15),bc+(lane>>4)*8,64));
  asm volatile("ldmatrix.sync.aligned.m8n8.x4.shared.b16 {%0,%1,%2,%3}, [%4];" : "=r"(ar_regs[0]),"=r"(ar_regs[1]),"=r"(ar_regs[2]),"=r"(ar_regs[3]):"r"(pa));
  asm volatile("ldmatrix.sync.aligned.m8n8.x4.trans.shared.b16 {%0,%1,%2,%3}, [%4];" : "=r"(br_regs[0]),"=r"(br_regs[1]),"=r"(br_regs[2]),"=r"(br_regs[3]):"r"(pb));
  mma16(z,ar_regs,br_regs);
 }
}
__global__ void inverse(const bf16* kp,const bf16* beta,const float* gp,const int* bounds,bf16* ap,int hv,int ns,int nc){
 ...
 // Combine only the lower blocks: two 16x16 connections, then one 32x32 connection.
 int warp=t>>5,lane=t&31;
 for(int x=t;x<4096;x+=512){int r=x/64,c=x%64;
  s.k[sw(r,c,64)]=bf(s.inv[x]);
  s.kt[sw(r,c,64)]=bf(-s.gram[x]);
 }
 __syncthreads();
 float z[8];
 // T16 = -Dlower * L10, one warp for each adjacent diagonal pair.
 if(warp<2){int g=warp*32;inverse_tile<16>(s.k,s.kt,g+16,g+16,g+16,g,z);}
 __syncthreads();
 if(warp<2){int g=warp*32;
  #pragma unroll
  for(int e=0;e<8;e++){
   int lr=(lane>>2)+((e&2)?8:0),lc=(lane&3)*2+(e&1)+((e&4)?8:0);
   s.kt[sw(g+16+lr,g+lc,64)]=bf(z[e]);
  }
 }
 __syncthreads();
 // C16 = T16 * Dupper. Preserve the FP32 result until operand publication.
 if(warp<2){int g=warp*32;inverse_tile<16>(s.kt,s.k,g+16,g,g,g,z);}
 __syncthreads();
 if(warp<2){int g=warp*32;
  #pragma unroll
  for(int e=0;e<8;e++){
   int lr=(lane>>2)+((e&2)?8:0),lc=(lane&3)*2+(e&1)+((e&4)?8:0);
   s.inv[(g+16+lr)*64+g+lc]=z[e];
  }
 }
 __syncthreads();
 for(int x=t;x<4096;x+=512){int r=x/64,c=x%64;
 ...
 __syncthreads();
 // T32 = -Rright * E, four warps cover the 32x32 cross block.
 if(warp<4){int m=(warp/2)*16,n=(warp%2)*16;inverse_tile<32>(s.k,s.kt,32+m,32,32,n,z);}
 __syncthreads();
 if(warp<4){int m=(warp/2)*16,n=(warp%2)*16;
  #pragma unroll
  for(int e=0;e<8;e++){
   int lr=(lane>>2)+((e&2)?8:0),lc=(lane&3)*2+(e&1)+((e&4)?8:0);
   s.kt[sw(32+m+lr,n+lc,64)]=bf(z[e]);
  }
 }
 __syncthreads();
 // C32 = T32 * Rleft completes the four remaining lower 16x16 blocks.
 if(warp<4){int m=(warp/2)*16,n=(warp%2)*16;inverse_tile<32>(s.kt,s.k,32+m,0,0,n,z);}
 __syncthreads();
 if(warp<4){int m=(warp/2)*16,n=(warp%2)*16;
  #pragma unroll
  for(int e=0;e<8;e++){
   int lr=(lane>>2)+((e&2)?8:0),lc=(lane&3)*2+(e&1)+((e&4)?8:0);
   s.inv[(32+m+lr)*64+n+lc]=z[e];
  }
 }
 __syncthreads();
 for(int x=t;x<4096;x+=512){int r=x/64,c=x%64;
  float inverse_value=s.inv[x];
  float weight=r>=c?expf(gp[(long(h)*nc+ch)*64+r]-gp[(long(h)*nc+ch)*64+c]):0.f;
  ap[(long(h)*nc+ch)*4096+x]=bf(inverse_value*weight);
 }
