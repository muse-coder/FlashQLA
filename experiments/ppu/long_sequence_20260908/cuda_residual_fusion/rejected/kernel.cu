...
__global__ void prepare_p(const bf16* qp,const bf16* kp,const float* gp,const int* bounds,bf16* pp,int hv,int ns,int nc){
...
template<class S> __global__ void fused_scan(const bf16* qp,const bf16* kp,const bf16* v,const bf16* beta,const float* gp,const bf16* ap,const bf16* pp,const S* initial,const int* bounds,bf16* output,float* final,int hv,int nc){
 ...
 for(int chunk=0;chunk<n;chunk++){
  int ch=chunk_base(left,seq)+chunk,start=left+64*chunk;long packed=long(h)*nc+ch,off=packed*8192;
  // QS and KS consume the same rounded pre-update state.
  #pragma unroll
  for(int j=0;j<2;j++){
  ...
  __syncthreads(); // B1: Q and Hbefore published.
  {
   float out[1][8];gemm<64,64,128>(s.a,s.b,out);
   #pragma unroll
   for(int e=0;e<8;e++){int r,c;coord<64>(0,e,r,c);out[0][e]*=expf(gp[packed*64+r]);}
   __syncthreads(); // B2: Q readers complete.
   #pragma unroll
   for(int j=0;j<2;j++){
    int group=t+j*512,row=group>>4,col=(group&15)*8;
    uint4 value=*reinterpret_cast<const uint4*>(kp+off+row*128+col);
    *reinterpret_cast<uint4*>(s.a+sw(row,col,128))=value;
   }
   __syncthreads(); // B3: K published; Hbefore unchanged.
   float work[1][8];gemm<64,64,128>(s.a,s.b,work);
   __syncthreads(); // B4: K and Hbefore readers complete.
   #pragma unroll
   for(int e=0;e<8;e++){
    int r,c;coord<64>(0,e,r,c);
    float residual_value=0.f;
    if(start+r<right){
     float value=fp(v[(long(start+r)*hv+h)*128+part*64+c]);
     residual_value=fmaf(-expf(gp[packed*64+r]),work[0][e],value);
    }
    s.b[sw(r,c,64)]=bf(residual_value);
   }
   for(int x=t;x<4096;x+=512){
    int r=x/64,c=x%64;
    float beta_col=start+c<right?fp(beta[long(start+c)*hv+h]):0.f;
    float ab_value=fp(ap[packed*4096+x])*beta_col;
    s.a[sw(r,c,64)]=bf(ab_value);
   }
   __syncthreads(); // B5: A_beta and residual published.
   gemm<64,64,64>(s.a,s.b,work);
   __syncthreads(); // B6: A_beta and residual readers complete.
   #pragma unroll
   for(int e=0;e<8;e++){int r,c;coord<64>(0,e,r,c);s.b[sw(r,c,64)]=bf(work[0][e]);}
   {int r=t>>3,c=(t&7)*8;
    uint4 pvalue=*reinterpret_cast<const uint4*>(pp+packed*4096+r*64+c);
    *reinterpret_cast<uint4*>(s.a+sw(r,c,64))=pvalue;
   }
   __syncthreads(); // B7: P and Vprime published.
   gemm<64,64,64>(s.a,s.b,work);
   #pragma unroll
   for(int e=0;e<8;e++){int r,c;coord<64>(0,e,r,c);if(start+r<right)output[(long(start+r)*hv+h)*128+part*64+c]=bf((out[0][e]+work[0][e])*0.08838834764831845f);}
   __syncthreads(); // B8: P readers complete; Vprime retained.
  }
  #pragma unroll
  for(int j=0;j<2;++j){
  ...
  __syncthreads(); // B9: natural KTD published.
  float delta[2][8];gemm_update_natural(s.a,s.b,delta);
  ...
  __syncthreads(); // B10: update readers complete before next chunk.
 }
 #pragma unroll
 for(int i=0;i<2;i++){
  #pragma unroll
...
std::vector<torch::Tensor> forward(torch::Tensor q,torch::Tensor k,torch::Tensor v,torch::Tensor g,torch::Tensor beta,torch::Tensor initial,torch::Tensor bounds){
 ...
 prepare_p<<<pg,512,0,stream>>>((bf16*)qp.data_ptr(),(bf16*)kp.data_ptr(),gp.data_ptr<float>(),bounds.data_ptr<int>(),(bf16*)pp.data_ptr(),hv,ns,nc);
 dim3 sg(hv*2,ns);
 #define STATE(ST) fused_scan<ST><<<sg,512,0,stream>>>((bf16*)qp.data_ptr(),(bf16*)kp.data_ptr(),(bf16*)v.data_ptr(),(bf16*)beta.data_ptr(),gp.data_ptr<float>(),(bf16*)ap.data_ptr(),(bf16*)pp.data_ptr(),(ST*)initial.data_ptr(),bounds.data_ptr<int>(),(bf16*)out.data_ptr(),last.data_ptr<float>(),hv,nc)
